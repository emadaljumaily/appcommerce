...
class CheckoutCardj extends StatefulWidget {
  final Cartj cart;
  final int total;
  final List<Cartj> list;
  const CheckoutCardj(this.total,this.list,  {
    Key key,
    this.cart
  }) : super(key: key);
  @override
...
class _CheckoutCard extends State<CheckoutCardj> {
  ...
  void by() async{
    ...
    for (int i = 0; i < widget.list.length; i++) {
      ...
      size=widget.list[i].size;
      final FirebaseUser user = await firebaseAuth.currentUser();
      String uid = user.uid;
      final ref = FirebaseDatabase.instance.reference().child('Jomlah');
